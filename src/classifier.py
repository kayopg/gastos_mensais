"""Classificacao de despesas em tres dimensoes: Categoria, Subcategoria e Tipo.

Hierarquia de prioridade na resolucao (do mais especifico ao mais generico):

  1. **Override por TRANSACAO** - `data/transaction_overrides.json`, keyed por
  ...
  2. **Override por ESTABELECIMENTO** - `data/categories.json`, keyed por nome
     do estabelecimento (para o caso comum: "GIASSI sempre eh Alimentacao").
  3. **Heuristicas automaticas** - `KEYWORD_RULES` (categoria) e
     `SUBCATEGORY_RULES` (subcategoria).
  4. **Defaults** - categoria=Outros, subcategoria="", tipo derivado de
     `is_parcelado` (Parcelado se True, senao "Variavel").
...
Estrutura de cada arquivo:
    ...
    transaction_overrides.json (transacao -> override granular)
...
# ---------------------------------------------------------------------------
# Persistencia por estabelecimento (categories.json)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Persistencia por transacao (transaction_overrides.json)
# ---------------------------------------------------------------------------
def tx_key(row) -> str:
    """Chave estavel para uma transacao. Usada como indice no JSON.

    Aceita dict, pd.Series ou objeto com __getitem__. Usa indexacao por
    chave (nao getattr) para evitar conflito com atributos nativos do
    pd.Series (ex: Series.data eh o buffer interno, nao a coluna).
    """
    def _g(k, default=""):
        try:
            v = row[k]
    ...
    try:
        valor_str = "{:.2f}".format(float(_g("valor", 0)))
...
# ---------------------------------------------------------------------------
# Heuristicas de keyword
# ---------------------------------------------------------------------------
def _match_keyword(estabelecimento: str, rules: dict):
...
# Resolucao
# ---------------------------------------------------------------------------
def resolve_no_tx(row, manual: ManualMap):
    """Resolve sem considerar override de transacao.

    Util para a UI saber qual seria o "auto" se o usuario limpasse o override.
    """
...
def _resolve_row(row, manual: ManualMap, tx_overrides: TxOverridesMap):
...
def classify(df: pd.DataFrame, manual_map=None, tx_overrides=None) -> pd.DataFrame:
